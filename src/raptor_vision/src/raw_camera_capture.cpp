#include "raptor_vision/raw_camera_capture.hpp"
#include "raptor_vision/cuda_utils.hpp"
#include <fcntl.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
#include <sys/epoll.h>
...
RawCameraCapturer::RawCameraCapturer(const PreprocessConfig& config)
    ...
    for (int i = 0; i < num_cameras_; i++) {
        if (i < (int)config.gain_map_paths.size() && !config.gain_map_paths[i].empty()) {
    ...
    for (int i = 0; i < num_cameras_; i++) {
        if (i < (int)config.rectify_map_paths.size() && !config.rectify_map_paths[i].empty()) {
